...
export function getGenericName(appName) {
    const genericAppName = appName.split("_")[0];
    return genericAppName;
}
export async function checkAuthorization(resourceNodeApp, requesterNodeApp, opts) {
    ...
    try {
        ...
        for (let page = 1; page <= lastPage; page += 1) {
            ...
            const { data: authPage } = await axios.get(url, {
            ...
            const activeAuthorizations = await Promise.all(items.map(async (authItem) => {
                const { data: authorization } = await axios.get(authItem.href, {
                    timeout: opts.timeout || AXIOS_TIMEOUT,
                });
...
export async function getApp(kid, opts) {
    let app;
    try {
        const response = await axios.get(kid, { timeout: opts?.timeout ?? AXIOS_TIMEOUT });
        app = response.data;
    }
    catch (error) {
        throw new Error(getErrorMessage(error));
...
export async function verifyJwtDid(jwt, opts) {
    ...
    if (doc.didDocument === null) {
        throw new Error(`Unable to resolve ${did}. Error: ${doc.didResolutionMetadata.error || "unknown error"}. Message: ${doc.didResolutionMetadata?.["message"] || "unknown details"}`);
    }
