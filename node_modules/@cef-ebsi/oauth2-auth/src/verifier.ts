...
import { importJWK, jwtVerify, type JWTVerifyResult, type KeyLike } from "jose";

export interface JwtTarVerifyResult extends JWTVerifyResult {
  publicKeyHex: string;
...
export async function verifyJwtTar(
  ...
  if (opts.op && opts.op !== getGenericName(payload.iss)) {
    throw new Error(
      `Invalid OpenID Provider. Received ${payload.iss}. Expected ${opts.op}`,
    );
  }
  ...
  // verify that the apps exist and are not revoked
  ...
  return valid.result as JwtTarVerifyResult;
}
